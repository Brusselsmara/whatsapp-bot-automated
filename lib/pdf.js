...
function buildRemittancePdf(txn) {
  return new Promise((resolve, reject) => {
    ...
    if (txn.payout_amount != null) {
      row('Amount paid to supplier:', `${txn.payout_amount} ${txn.currency}`);
      row('Yellow Card POBO fee:', `${txn.yc_fee_amount || 0} ${txn.currency}`);
      row('PayLink service fee:', `${txn.markup_amount || 0} ${txn.currency}`);
