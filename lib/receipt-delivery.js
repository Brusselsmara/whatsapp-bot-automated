...
async function deliverSendReceipt(txn) {
  if (!txn || txn.receipt_sent || txn.status !== 'completed') {
  ...
  const row = fresh || txn;
  if (!publicAppUrl()) {
    console.error(`[RECEIPT] PUBLIC_APP_URL not set — cannot attach PDF for txn ${txn.id}`);
  ...
  console.log(`[RECEIPT] Attaching PDF for txn ${txn.id}: ${receiptUrl}`);
  ...
  try {
    ...
    console.log(`[RECEIPT] ✅ Delivered for txn ${txn.id}`);
    return { sent: true };
  } catch (e) {
    console.error(`[RECEIPT] WhatsApp send failed for ${txn.id}:`, e.message);
    await supabase.from('transactions').update({ receipt_sent: false }).eq('id', txn.id);
