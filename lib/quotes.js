...
//  2. Cross-border — wallet currency != recipient currency. YC disbursement
//     fee is looked up on the DESTINATION leg (country/currency/payout amount),
//     converted to the sender's wallet currency, then marked up by 15%.
//     FX uses YC /business/rates bridged via USD with 2% margin shaved off.
// ============================================================
...
/**
 * YC send fee from POST /business/fees/get-config for a given leg.
 * @param {{ required?: boolean }} opts - when required, throws instead of returning 0
 */
async function getYcSendFeeAmount({ country, currency, channelType, amount }, opts = {}) {
  ...
  try {
    ...
    if (!svc) {
      if (required) throw new Error(`No YC fee config for send ${country}/${currency}/${channelType}`);
      return 0;
    }
    return parseFloat(
...
async function buildCrossBorderEstimate({ accountType, sourceCurrency, sourceCountry, destCurrency, destCountry, channelType, sourceAmount }) {
  const principal = parseFloat(sourceAmount);

  // 1) YC FX bridge (GET /business/rates) with PayLink's 2% margin on the rate
  const { rate1, rate2, bridgedRate, marginPct, displayRate } = await getBridgedRate({
    accountType, sourceCurrency, destCurrency, destCountry, sourceAmountForVip: principal,
  });
  const destAmountEstimate = parseFloat((principal * displayRate).toFixed(2));

  // 2) YC disbursement fee on the destination leg (what YC actually charges)
  const ycFeeDest = await getYcSendFeeAmount(
    { country: destCountry, currency: destCurrency, channelType, amount: destAmountEstimate },
    { required: true }
  );
  const ycFeeWallet = destFeeToWalletCurrency(ycFeeDest, displayRate);

  // 3) PayLink markup on the wallet-equivalent YC fee (default 15%)
...
function formatCrossBorderQuoteMessage(ctx) {
  ...
  return (
    ...
    `Fee: *${customerFee.toFixed(2)} ${ctx.walletCurrency}*\n\n` +
