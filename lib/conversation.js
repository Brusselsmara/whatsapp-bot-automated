...
const { buildInvoicePaymentQuote, formatInvoiceQuoteMessage } = require('./fees');
const {
  claimTopupCredit,
  claimSendComplete,
  claimSendRefund,
...
async function settleTopup(txn) {
  ...
  if (['COMPLETE', 'COMPLETED', 'SUCCESS'].includes(ycStatus)) {
    ...
    console.log(`[SETTLE] ✅ Topup credited ${result.amount} ${result.currency} — balance ${result.newBalance}`);
    sendWhatsApp(txn.phone,
      `✅ Top-up of *${result.amount} ${result.currency}* confirmed! Your new balance is *${result.newBalance} ${result.currency}*.`
...
async function handleInvoicePayAccount(phone, msg, session) {
  ...
  try {
    ...
    const { data: wallet } = await supabase
      ...
      .eq('phone', phone).eq('currency', ctx.currency).single();
    const balance = wallet ? parseFloat(wallet.balance) : 0;
    if (balance < quote.totalCharge) {
      await setSession(phone, 'idle', {});
      return `Insufficient balance for this invoice.\n\nYou need *${quote.totalCharge.toFixed(2)} ${ctx.currency}* (incl. fees) but only have *${balance.toFixed(2)} ${ctx.currency}*.\n\nReply *menu* → Top-up Balance to add funds.`;
    }
    ...
    await setSession(phone, 'txn_amount', {
      ...
      ycFeeAmount: quote.ycFeeAmount,
      markupAmount: quote.markupAmount,
      amount: quote.totalCharge,
    });
    return formatInvoiceQuoteMessage(ctx, quote);
  } catch (err) {
    console.error('[INVOICE] Quote error:', err);
    return `Could not calculate the payment quote right now (${err.message}). Reply *menu* to try again.`;
  }
}
...
async function handleTxnAmount(phone, msg, session) {
  ...
  // Invoice lookup flow — quote already shown; wait for confirm/cancel
  if (txnCtx.invoiceCode && txnCtx.amount && txnCtx.payoutAmount != null) {
    if (msg.trim().toLowerCase() === 'cancel') {
      await setSession(phone, 'idle', {});
      return "Payment cancelled. Reply *menu* to go back.";
    }
    if (msg.trim().toLowerCase() !== 'confirm') {
      return formatInvoiceQuoteMessage(txnCtx, {
        payoutAmount: txnCtx.payoutAmount,
        ycFeeAmount: txnCtx.ycFeeAmount,
        markupAmount: txnCtx.markupAmount,
        totalCharge: txnCtx.amount,
      });
    }
  ...
  // Manual invoice payment — build fee quote before debiting wallet
  if (ctx.purpose === 'invoice_payment') {
    try {
      ...
      if (balance < quote.totalCharge) {
        await setSession(phone, 'idle', {});
        return `Insufficient balance.\n\nYou need *${quote.totalCharge.toFixed(2)} ${ctx.currency}* (incl. fees) but only have *${balance.toFixed(2)} ${ctx.currency}*. Reply *menu* → Top-up Balance.`;
      }
      ...
      await setSession(phone, 'txn_reference', quoted);
  ...
  return finalizeTransaction(phone, { ...ctx, amount }, null);
}
...
async function finalizeTransaction(phone, ctx, reference) {
  ...
  const payoutAmount = isInvoice ? parseFloat(ctx.payoutAmount || ctx.amount) : debitAmount;
  ...
  try {
    ...
    await safeDebitWallet(phone, ctx.currency, debitAmount);
    walletDebited = true;
    ...
    const sender = {
      ...
      country,
      phone: yc.toInternationalPhone(phone, country),
    ...
    if (send?.id && ['created', 'pending'].includes((send.status || '').toLowerCase())) {
      try {
        await yc.acceptSend(send.id);
        console.log(`[SEND] acceptSend called for ${send.id}`);
      } catch (e) {
        console.warn(`[SEND] acceptSend fallback skipped:`, e.message);
      }
    }
    ...
    await supabase.from('transactions').insert({
      ...
      amount: debitAmount,
      payout_amount: isInvoice ? payoutAmount : null,
    ...
    const feeNote = isInvoice
      ? `\n(Supplier receives ${payoutAmount} ${ctx.currency}; ${debitAmount} ${ctx.currency} debited incl. fees)`
      : '';
    ...
    if (walletDebited) {
      ...
      return `❌ Payment failed after debiting your wallet. Your ${ctx.currency} ${debitAmount} has been refunded. Please try again.`;
    }
...
function formatTxnStatus(t) {
  ...
  const pendingNote = !['completed', 'failed'].includes(t.status)
    ...
    : '';
  const feeLines = t.type === 'invoice_payment' && t.payout_amount != null
    ? `Supplier receives: ${parseFloat(t.payout_amount).toFixed(2)} ${t.currency}\n` +
      `Fees included: ${(parseFloat(t.yc_fee_amount || 0) + parseFloat(t.markup_amount || 0)).toFixed(2)} ${t.currency}\n` +
