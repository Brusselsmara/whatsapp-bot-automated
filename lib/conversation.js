...
const { findPaylinkRecipient, executeP2PTransfer, P2P_MAX_AMOUNT } = require('./p2p');
const {
...
7️⃣ Create Invoice
8️⃣ Send to PayLink user (free & instant)`;
...
5️⃣ Send to PayLink user (free & instant)`;
...
async function handleIncomingMessage(phone, text, mediaRefs = [], opts = {}) {
  ...
  switch (session.state) {
    ...
    // PayLink-to-PayLink (internal wallet transfer)
...
async function actionSendPaylink(phone) {
  const user = await getOrCreateUser(phone);
  if (user.kyc_status !== 'approved') {
    return 'Your account must be approved before you can send to other PayLink users.';
  }
  if (!user.home_currency) {
  ...
  return (
    `*Send to PayLink user* — free & instant in *${user.home_currency}*.\n\n` +
    `Enter the recipient's PayLink phone number with country code (e.g. +26771234567):`
  );
}
...
async function handleP2pRecipientPhone(phone, msg, session) {
  ...
  if (!recipientPhone || !yc.isSupportedWhatsAppNumber(recipientPhone)) {
    return 'Enter a valid PayLink phone number with country code, e.g. +26771234567.';
  }
  if (recipientPhone === normalizePhone(phone)) {
    return 'You cannot send money to yourself. Enter a different PayLink number.';
  }

  const recipient = await findPaylinkRecipient(recipientPhone);
  if (!recipient) {
    return (
      `*${recipientPhone}* is not on PayLink yet, or their account is not approved.\n\n` +
      `Ask them to register on WhatsApp first, or reply *menu* → Send money to bank/${MOMO_LABEL}.`
    );
  }
  if (recipient.home_currency !== currency) {
    return (
      `Cross-currency PayLink transfers aren't available yet.\n\n` +
...
async function handleP2pAmount(phone, msg, session) {
  ...
  if (amount > P2P_MAX_AMOUNT) {
    return `Maximum PayLink-to-PayLink transfer is ${P2P_MAX_AMOUNT} ${session.context.currency}. Reply *menu* for bank/${MOMO_LABEL} sends.`;
  }
...
async function routeHome(phone, user, msg) {
  ...
  if (user.kyc_status === 'pending_review') {
    return "Your registration is still under review. We'll notify you in the PayLink app when it's approved — usually within 1 business day.";
  }
...
async function finalizeRegistration(phone, ctx, documentUrls) {
  ...
  return (
    ...
    `We'll notify you in the *PayLink app* within 1 business day when your review is complete.\n\n` +
...
Your balance will update automatically once your payment is confirmed.${sandboxNote}`;
    ...
    if (errMsg.includes('Full KYC information')) {
      const fullKycNote = yc.requiresFullKycForCurrency(ctx.currency)
        ? `\n\n_${ctx.currency} top-ups require complete identity details on your PayLink profile._`
        : '';
      return (
...
function formatTxnStatus(t) {
  const typeLabel = { topup: 'Top-up', send: 'Transfer', invoice_payment: 'Invoice payment', p2p: 'PayLink transfer' };
...
async function handleTransactionHistory(phone) {
  ...
  const typeLabel = { topup: '⬇️ Top-up', send: '⬆️ Transfer', invoice_payment: '🧾 Invoice', p2p: '⚡ PayLink' };
