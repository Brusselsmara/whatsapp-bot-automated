...
const { notifyUser, stripMarkdown, normalizePhone, resolveStoredPhone } = require('./notifications');
const { enqueueAppMessage, formatSendFailedAppMessage } = require('./app-messages');
...
const { buildInvoicePaymentUrl, loadInvoiceRow, getMerchantDisplayName } = require('./invoices');
const { findRomelaPulaRecipient, executeP2PTransfer, P2P_MAX_AMOUNT } = require('./p2p');
const {
...
const WELCOME = `Welcome to *Romela Pula* 👋
...
7️⃣ Create Invoice
8️⃣ Send to Romela Pula user (free & instant)`;
...
5️⃣ Send to Romela Pula user (free & instant)`;
...
function supportedRegistrationCountriesMessage() {
  ...
  return (
    `Romela Pula is available for WhatsApp numbers from these countries:\n` +
    `${lines.join('\n')}\n\n` +
    `Please use a WhatsApp account registered with one of these country codes.`
  );
}

function romelaPulaCorridorSummary() {
  const count = yc.getRegisterableCorridors().length;
  return `Romela Pula lets you pay invoices and send money across ${count} African countries.`;
}

/** User-facing copy when the payments backend is down — never mention third-party providers. */
function romelaPulaUnavailableMsg() {
  return 'Romela Pula is temporarily unavailable for this transaction. Please wait a moment and try again, or reply *menu*.';
}

function romelaPulaPartnerError(err, retryHint = 'Reply *menu* to try again.') {
  const msg = String(err?.message || '');
  if (err?.status >= 500 || err?.status === 429 || msg.includes('timed out')) {
    return romelaPulaUnavailableMsg();
  }
  return `Romela Pula couldn't complete this step right now. ${retryHint}`;
}
 ...
 */
async function resolvePhoneKeys(phone) {
  phone = normalizePhone(phone);
  ...
  return [...new Set([storedPhone, phone, storedPhone.replace(/^\+/, ''), phone.replace(/^\+/, '')])]
    .filter(Boolean);
}

/** Pending YC settlement or undelivered PDF receipt — used by PWA background sync. */
async function getSettlementPendingCount(phone) {
  const phoneKeys = await resolvePhoneKeys(phone);
  ...
  const { count: undeliveredReceipts } = await supabase
    .from('transactions')
    .select('id', { count: 'exact', head: true })
    .in('phone', phoneKeys)
  ...
  return (active || 0) + (undeliveredReceipts || 0);
}
  ...
  if (['COMPLETE', 'COMPLETED', 'SUCCESS', 'SUCCESSFUL', 'SETTLED'].includes(ycStatus)) {
    try {
      ...
      try {
        ...
        console.error('[SETTLE] Topup notify failed:', e.message);
      }
    } catch (err) {
      console.error(
    ...
    try {
      ...
      console.error('[SETTLE] Topup fail notify failed:', e.message);
    }
  }
}
...
async function settleSend(txn) {
  ...
  if (SEND_COMPLETE.includes(ycStatus)) {
    ...
    try {
      ...
      console.error('[SETTLE] Refund message failed:', e.message);
    }
  }
}
 ...
 * @param {string[]} mediaRefs
 * @param {{ channel?: 'whatsapp' | 'pwa' }} opts — pwa: never activates CSW from chat
 */
async function handleIncomingMessage(phone, text, mediaRefs = [], opts = {}) {
  phone = normalizePhone(phone);
  ...
  if (csw.isPwaActivationKeyword(lower)) {
    if (channel === 'pwa') {
      return csw.buildPwaActivationHintForApp(phone, user);
    }
  ...
  switch (session.state) {
    ...
    case 'txn_bank_name':
      return handleTxnBankName(phone, msg, session);
    case 'txn_bank_account_name':
      return handleTxnBankAccountName(phone, msg, session);
    case 'txn_bank_branch':
      return handleTxnBankBranch(phone, msg, session);
    case 'txn_currency':
    ...
    // Romela Pula user-to-user (internal wallet transfer)
    case 'p2p_recipient_phone':
      return handleP2pRecipientPhone(phone, msg, session);
    case 'p2p_amount':
      return handleP2pAmount(phone, msg, session);
    case 'p2p_confirm':
      return handleP2pConfirm(phone, msg, session);
...
async function actionSendRomelaPula(phone) {
  const user = await getOrCreateUser(phone);
  if (user.kyc_status !== 'approved') {
    return 'Your account must be approved before you can send to other Romela Pula users.';
  }
  if (!user.home_currency) {
    return `We couldn't work out your wallet currency. Contact support if this persists.`;
  }
  await setSession(phone, 'p2p_recipient_phone', { currency: user.home_currency });
  return (
    `*Send to Romela Pula user* — free & instant in *${user.home_currency}*.\n\n` +
    `Enter the recipient's Romela Pula phone number with country code (e.g. +26771234567):`
  );
}

async function handleP2pRecipientPhone(phone, msg, session) {
  const sender = await getOrCreateUser(phone);
  const currency = session.context.currency || sender.home_currency;
  const recipientPhone = normalizePhone(msg.trim());

  if (!recipientPhone || !yc.isSupportedWhatsAppNumber(recipientPhone)) {
    return 'Enter a valid Romela Pula phone number with country code, e.g. +26771234567.';
  }
  if (recipientPhone === normalizePhone(phone)) {
    return 'You cannot send money to yourself. Enter a different Romela Pula number.';
  }

  const recipient = await findRomelaPulaRecipient(recipientPhone);
  if (!recipient) {
    return (
      `*${recipientPhone}* is not on Romela Pula yet, or their account is not approved.\n\n` +
      `Ask them to register on WhatsApp first, or reply *menu* → Send money to bank/${MOMO_LABEL}.`
    );
  }
  if (recipient.home_currency !== currency) {
    return (
      `Cross-currency Romela Pula transfers aren't available yet.\n\n` +
      `This recipient uses *${recipient.home_currency}*; your wallet is *${currency}*. ` +
      `Reply *menu* → Send money for a cross-border payout instead.`
    );
  }

  const displayName = recipient.business_name || recipient.kyc_name || recipient.phone;
  await setSession(phone, 'p2p_amount', {
    currency,
    recipientPhone: recipient.phone,
    recipientName: displayName,
  });
  return `How much *${currency}* would you like to send to *${displayName}* (${recipient.phone})?\n\nEnter just the number (max ${P2P_MAX_AMOUNT} ${currency} per transfer).`;
}

async function handleP2pAmount(phone, msg, session) {
  const amount = parseFloat(msg);
  if (isNaN(amount) || amount <= 0) {
    return "That doesn't look like a valid amount. Please enter a number, e.g. 100.";
  }
  if (amount < MIN_AMOUNT) {
    return `Minimum transfer amount is ${MIN_AMOUNT}. Please enter a larger amount.`;
  }
  if (amount > P2P_MAX_AMOUNT) {
    return `Maximum Romela Pula user-to-user transfer is ${P2P_MAX_AMOUNT} ${session.context.currency}. Reply *menu* for bank/${MOMO_LABEL} sends.`;
  }
  ...
  if (balance < amount) {
    await setSession(phone, 'idle', {});
    return `Insufficient balance. You have *${balance.toFixed(2)} ${session.context.currency}* but tried to send *${amount.toFixed(2)}*. Reply *menu* → Top-up Balance.`;
  }

  await setSession(phone, 'p2p_confirm', { ...session.context, amount });
  return (
    `*Confirm Romela Pula transfer*\n\n` +
    `To: *${session.context.recipientName}* (${session.context.recipientPhone})\n` +
    `Amount: *${amount.toFixed(2)} ${session.context.currency}*\n` +
    `Fee: *Free*\n\n` +
    `Reply *confirm* to send instantly, or *cancel*.`
  );
}

async function handleP2pConfirm(phone, msg, session) {
  const lower = msg.trim().toLowerCase();
  if (lower === 'cancel') {
    await setSession(phone, 'idle', {});
    return 'Transfer cancelled. Reply *menu* to see options.';
  }
  if (lower !== 'confirm') {
    return (
      `Reply *confirm* to send *${session.context.amount} ${session.context.currency}* to ` +
      `*${session.context.recipientName}*, or *cancel*.`
    );
  }
  ...
  const senderUser = await getOrCreateUser(phone);
  const senderName = senderUser.business_name || senderUser.kyc_name || phone;
  try {
    await enqueueAppMessage(phone, {
      text:
        `✅ *Transfer sent*\n\n` +
        `${ctx.amount.toFixed(2)} ${ctx.currency} sent to ${ctx.recipientName}.\n` +
        `Your new balance: ${result.senderBalance.toFixed(2)} ${ctx.currency}.`,
    });
    await enqueueAppMessage(ctx.recipientPhone, {
      text:
        `💰 *Money received*\n\n` +
        `${ctx.amount.toFixed(2)} ${ctx.currency} from ${senderName}.\n` +
        `Your new balance: ${result.recipientBalance.toFixed(2)} ${ctx.currency}.`,
    });
  } catch (e) {
    console.warn('[P2P] Notification failed:', e.message);
  }

  return (
    `✅ *${ctx.amount.toFixed(2)} ${ctx.currency}* sent to *${ctx.recipientName}* instantly.\n\n` +
    `Your new balance: *${result.senderBalance.toFixed(2)} ${ctx.currency}*.\n\n` +
...
async function handleInvoicePayCode(phone, msg, session) {
  ...
  return beginInvoicePayment(phone, msg.trim());
}

/** Start paying an invoice by code — used by WhatsApp, PWA deep links, and /api/app. */
async function beginInvoicePayment(phone, rawCode) {
  ...
  const supplierName = await getMerchantDisplayName(invoice.issuer_phone);
  ...
  if (!country) {
    const channels = yc.sendChannelTypesForCurrency(invoice.currency);
    const baseCtx = {
    ...
    if (channels.length === 0) {
      return `${summary}\n\nSorry, this invoice currency isn't supported for payouts right now. Reply *menu* to go back.`;
    }
    if (channels.length === 1) {
      await setSession(phone, 'invoice_pay_country', { ...baseCtx, channelType: channels[0] });
      return (
        `${summary}\n\n` +
        yc.formatCorridorPickerPrompt(
          ...
          { currency: invoice.currency, channelType: channels[0] }
        )
      );
    }
    await setSession(phone, 'invoice_pay_channel_choice', baseCtx);
    return `${summary}\n\n${formatPayChannelPrompt(channels)}`;
  }
  const ctx = {
    purpose: 'invoice_payment',
    invoiceCode: code,
  ...
  return `${summary}\n\n${formatPayChannelPrompt(allowed)}`;
}

async function handleInvoicePayCountry(phone, msg, session) {
  const pickerOpts = {
    currency: session.context.currency,
    ...(session.context.channelType ? { channelType: session.context.channelType } : {}),
  };
  const corridor = yc.parseCorridorPickerChoice(msg, pickerOpts);
  if (!corridor) {
    return (
  ...
  if (ctx.channelType) {
    if (!allowed.includes(ctx.channelType)) {
      const allowedLabel = allowed.map((c) => channelLabelInline(c)).join(' or ');
      return `${corridor.name} only supports ${allowedLabel} for sends. Reply *menu* and try again.`;
    }
    await setSession(phone, 'invoice_pay_account', ctx);
    const label = ctx.channelType === 'bank' ? "supplier's bank account number" : `supplier's ${MOMO_LABEL} number`;
    return `${summary}\n\nWhat's the ${label}?`;
  }
...
async function handleInvoicePayChannelChoice(phone, msg, session) {
  const ctx = session.context;
  const allowed = ctx.country
    ? sendChannelTypesFor(ctx.country)
    : yc.sendChannelTypesForCurrency(ctx.currency);
  const channelType = parsePayChannelChoice(msg, allowed);
  if (!channelType) {
    return `Please choose a valid option.\n\n${formatPayChannelPrompt(allowed)}`;
  }

  if (ctx.country && !allowed.includes(channelType)) {
    const allowedLabel = allowed.map((c) => channelLabelInline(c)).join(' or ');
    return `${ctx.currency} only supports ${allowedLabel}. Please choose again.`;
  }

  const nextCtx = { ...ctx, channelType };
  if (!ctx.country) {
    await setSession(phone, 'invoice_pay_country', nextCtx);
    return yc.formatCorridorPickerPrompt(
      `Which country is the supplier in? (invoice currency: *${ctx.currency}*)`,
      { currency: ctx.currency, channelType }
    );
  }

  await setSession(phone, 'invoice_pay_account', nextCtx);
 ...
 * bridge, with Romela Pula markups/margins bundled into the customer Fees line.
 */
...
async function handleInvoicePayAccount(phone, msg, session) {
  ...
  if (ctx.channelType === 'bank') {
    await setSession(phone, 'txn_bank_name', { ...ctx, recipientAccountNumber });
    return bankDetailPrompt('name', ctx.purpose);
  }
  ...
  try {
    ...
    return romelaPulaPartnerError(err);
  }
}
...
async function handleInvoicePayRecipientConfirm(phone, msg, session) {
  ...
  try {
    ...
    return romelaPulaPartnerError(err, 'Reply *menu* to try again.');
  }
}
...
async function routeHome(phone, user, msg) {
  if (user.kyc_status === 'unregistered' || user.kyc_status === 'rejected') {
    ...
    if (msg === '2') {
      await setSession(phone, 'idle', {});
      return `${romelaPulaCorridorSummary()} Reply "1" to register and get started.`;
    }
  ...
  if (user.kyc_status === 'pending_review') {
    return "Your registration is still under review. We'll notify you in the Romela Pula app when it's approved — usually within 1 business day.";
  }
...
async function handleRegisterName(phone, msg, session) {
  ...
  return 'Date of birth? (dd/mm/yyyy, e.g. 15/03/1990)';
}
...
async function handleRegisterDob(phone, msg, session) {
  ...
  const dobForYc = yc.formatDobForYc(raw);
  if (!dobForYc) {
    return 'That date of birth is not valid (must be in the past). Please use dd/mm/yyyy, e.g. 15/03/1990.';
  }
  await setSession(phone, 'register_address', { ...session.context, dob: dobForYc });
...
async function finalizeRegistration(phone, ctx, documentUrls) {
  ...
  return (
    ...
    `We'll notify you in the *Romela Pula app* within 1 business day when your review is complete.\n\n` +
    `If we need anything else, you'll receive a notification in the app.`
  );
}
...
function formatPayChannelPrompt(channels) {
  const lines = [];
  let n = 1;
  if (channels.includes('bank')) {
    lines.push(`${n}️⃣ Bank transfer`);
    n += 1;
  }
  if (channels.includes('momo')) {
    lines.push(`${n}️⃣ ${MOMO_LABEL}`);
  }
  return `Pay via:\n${lines.join('\n')}`;
}

function parsePayChannelChoice(msg, channels) {
  const choice = String(msg || '').trim();
  let n = 1;
  if (channels.includes('bank')) {
    if (choice === String(n)) return 'bank';
    n += 1;
  }
  if (channels.includes('momo')) {
    if (choice === String(n)) return 'momo';
  }
  return null;
}

function bankDetailPrompt(field, purpose = 'send') {
  const who = purpose === 'invoice_payment' ? 'Supplier' : 'Recipient';
  if (field === 'name') return `${who}'s bank name?`;
  if (field === 'accountName') return 'Account holder name (as it appears on the bank account)?';
  if (field === 'branch') return 'Branch name?';
  return 'Please provide the requested bank detail.';
}

function formatBankRecipientLines(ctx) {
  if (ctx.channelType !== 'bank') return [];
  const lines = [];
  if (ctx.bankName) lines.push(`Bank: *${ctx.bankName}*`);
  if (ctx.recipientBankAccountName) lines.push(`Account name: *${ctx.recipientBankAccountName}*`);
  if (ctx.branchName) lines.push(`Branch: *${ctx.branchName}*`);
  if (ctx.recipientAccountNumber) lines.push(`Account number: *${ctx.recipientAccountNumber}*`);
  return lines;
}

async function proceedToBankRecipientVerification(phone, ctx) {
  const { country, recipientAccountNumber } = ctx;
  const recipientLabel = ctx.purpose === 'invoice_payment' ? 'supplier' : 'recipient';
  const confirmState = ctx.purpose === 'invoice_payment' ? 'invoice_pay_recipient_confirm' : 'txn_recipient_confirm';
  const accountState = ctx.purpose === 'invoice_payment' ? 'invoice_pay_account' : 'txn_recipient_account';

  try {
    const resolution = await resolveRecipientIdentity({
      channelType: 'bank',
      country,
      accountNumber: recipientAccountNumber,
    });
    if (resolution.notFound) {
      await setSession(phone, accountState, ctx);
      return `We couldn't find an account matching that number. Please double-check the account number and try again.`;
    }
    const nextCtx = {
      ...ctx,
      ...(resolution.name && !ctx.recipientBankAccountName ? { recipientName: resolution.name } : {}),
    };
    await setSession(phone, confirmState, nextCtx);
    return formatRecipientConfirmMessage(nextCtx, resolution, recipientLabel);
  } catch (err) {
...
function formatRecipientConfirmMessage(ctx, resolution, recipientLabel = 'recipient') {
  const bankLines = formatBankRecipientLines(ctx);
  if (bankLines.length) {
    return (
      `Please confirm:\n\n${bankLines.join('\n')}\n\n` +
...
async function handleTxnBankName(phone, msg, session) {
  const bankName = msg.trim();
  if (!bankName) return bankDetailPrompt('name', session.context.purpose);
  await setSession(phone, 'txn_bank_account_name', { ...session.context, bankName });
  return bankDetailPrompt('accountName', session.context.purpose);
}

async function handleTxnBankAccountName(phone, msg, session) {
  const recipientBankAccountName = msg.trim();
  if (!recipientBankAccountName) return bankDetailPrompt('accountName', session.context.purpose);
  await setSession(phone, 'txn_bank_branch', { ...session.context, recipientBankAccountName });
  return bankDetailPrompt('branch', session.context.purpose);
}

async function handleTxnBankBranch(phone, msg, session) {
  const branchName = msg.trim();
  if (!branchName) return bankDetailPrompt('branch', session.context.purpose);
  const ctx = { ...session.context, branchName };

  if (ctx.country && ctx.currency) {
    return proceedToBankRecipientVerification(phone, ctx);
  }

  await setSession(phone, 'txn_currency', ctx);
  return yc.formatCorridorPickerPrompt('Which country is the recipient in?', { channelType: 'bank' });
}
...
async function handleTxnRecipientAccount(phone, msg, session) {
  ...
  if (ctx.channelType === 'momo') {
    ...
    if (!detected) {
      const supported = yc.getSendCorridors()
        .filter((c) => (c.channelTypes || []).includes('momo'))
        .map((c) => {
          const dial = yc.COUNTRY_CONFIG[c.country]?.dialCode;
          return dial ? `+${dial} (${c.currency})` : `${c.name} (${c.currency})`;
        })
        .join(', ');
  ...
  // Bank — collect bank details, then country (filtered to bank corridors).
  await setSession(phone, 'txn_bank_name', { ...ctx, recipientAccountNumber: accountNumber });
  return bankDetailPrompt('name', ctx.purpose);
}

async function handleTxnCurrency(phone, msg, session) {
  const pickerOpts = { channelType: session.context.channelType || 'bank' };
  const corridor = yc.parseCorridorPickerChoice(msg, pickerOpts);
  if (!corridor) {
    return (
      `Please reply with a valid number from the list.\n\n` +
      yc.formatCorridorPickerPrompt('Which country is the recipient in?', pickerOpts)
    );
  }
...
async function handleTxnQuoteConfirm(phone, msg, session) {
  ...
  if (lower === '1') {
    try {
      ...
      return romelaPulaPartnerError(err, 'Reply *1* to try again or *cancel*.');
    }
  }
...
async function handleTxnAmount(phone, msg, session) {
  ...
  if (txnCtx.purpose === 'invoice_payment' && !txnCtx.invoiceCode) {
    try {
      ...
      return `${formatSendQuoteMessage(feeCtx, invoiceQuoteFromCtx(feeCtx))}\n\nWhat reference should appear on your payment receipt? (e.g. your invoice number or Purchase Order number)`;
    } catch (err) {
...
async function finalizeTransaction(phone, ctx, reference) {
  ...
  try {
    ...
    const sender = yc.buildRetailKycFromUser(user, phone, senderCountry, { strict: true })
      || yc.buildRetailKycFromUser(user, phone, senderCountry);
    ...
    const send = await yc.submitSend({
      ...
      sender,
      destination: {
        accountName: ctx.recipientBankAccountName || ctx.recipientName,
        accountNumber: ctx.recipientAccountNumber,
        accountType: ctx.channelType,
        networkId,
        country,
        ...(ctx.bankName ? { accountBank: ctx.bankName } : {}),
      },
    });
    ...
    const feeNote = (isInvoice || isPlainSend)
      ...
      : '';
    return `✅ ${label} of ${payoutAmount} ${ctx.currency} to ${ctx.recipientName} initiated. Reference: ${send.id}${feeNote}\n\n⏳ Your wallet has been debited. We're processing the payout — you'll get your PDF receipt here in chat as soon as it's confirmed (no need to send another message).`;
  } catch (err) {
...
async function submitTopupReceive(phone, ctx, momoOpts = null) {
  ...
  const missingKyc = yc.describeMissingKycFields(user);
  if (missingKyc.length > 0) {
    return (
      `Your profile is missing details required for top-up:\n` +
      missingKyc.map((f) => `• ${f}`).join('\n') +
      `\n\nContact support to update your registration details, or reply *menu* and register again if needed.`
    );
  }

  const recipient = yc.buildRetailKycFromUser(user, phone, ctx.country, { strict: true });
  if (!recipient) {
    return (
      `We could not validate your profile for this top-up (check date of birth and ID details). ` +
      `Contact support or reply *menu* to register again with correct information.`
    );
  }

  const kycIssues = yc.validateRetailKycPayload(recipient);
  if (kycIssues.length > 0) {
    return (
      `Your profile could not be verified for this top-up:\n` +
      kycIssues.map((f) => `• ${f}`).join('\n') +
      `\n\nContact support to update your details, or reply *menu* to register again.`
    );
  }
  ...
  try {
    console.log(
      `[TOPUP] submitReceive payload — source:`,
      JSON.stringify(source),
      'recipient:',
      JSON.stringify({ ...recipient, idNumber: '***' })
    );
...
Your balance will update automatically once your payment is confirmed.${sandboxNote}`;
    ...
    if (errMsg.includes('Full KYC information')) {
      const fullKycNote = yc.requiresFullKycForCurrency(ctx.currency)
        ? `\n\n_${ctx.currency} top-ups require complete identity details on your Romela Pula profile._`
        : '';
      return (
        `Your profile details could not be verified for this top-up. ` +
        `Check that your date of birth (dd/mm/yyyy), ID type (National ID or Passport), and ID number are correct. ` +
        `Contact support to update your profile, or reply *menu* to register again.${fullKycNote}`
      );
    }
    return romelaPulaUnavailableMsg();
  }
...
function formatTxnStatus(t) {
  const typeLabel = { topup: 'Top-up', send: 'Transfer', invoice_payment: 'Invoice payment', p2p: 'Romela Pula transfer' };
...
async function handleTransactionHistory(phone) {
  ...
  const typeLabel = { topup: '⬇️ Top-up', send: '⬆️ Transfer', invoice_payment: '🧾 Invoice', p2p: '⚡ Romela Pula' };
...
async function handleInvoiceCreateDescription(phone, msg, session) {
  ...
  const paymentUrl = buildInvoicePaymentUrl(invoiceCode);
  const linkLine = paymentUrl
    ? `\n*Payment link:* ${paymentUrl}\n_Share this link — customers can pay from any browser._\n`
    : '\n_Set PUBLIC_APP_URL on your deployment to enable shareable payment links._\n';
  ...
  return (
    ...
    `*Description:* ${msg.trim()}\n` +
    linkLine +
    `\nCustomers can also pay by messaging this WhatsApp number → option 1️⃣ (Pay Invoice) and entering the code.\n\n` +
