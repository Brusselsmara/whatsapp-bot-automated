...
function multiplyByRate(amount, rate, rateDecimals = 6) {
  ...
  const rateInt = Math.round(parseFloat(rate) * rateFactor);
  if (rateInt <= 0) throw new Error('Invalid rate');
  return fromMinor(Math.round((amountMinor * rateInt) / rateFactor));
}
...
function divideByRate(amount, rate, rateDecimals = 6) {
  ...
  const rateInt = Math.round(parseFloat(rate) * rateFactor);
  if (rateInt <= 0) throw new Error('Invalid rate');
  return fromMinor(Math.round((amountMinor * rateFactor) / rateInt));
}
