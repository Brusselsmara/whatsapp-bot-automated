...
async function backfillUserHomeCurrency(phone, user) {
  ...
  if (!detected) {
    console.warn(`[USER] Could not resolve home currency for ${phone}`);
    return user;
  }

  const { data: updated, error } = await supabase
    .from('users')
    .update({ home_currency: detected.currency, home_country: detected.country })
    .eq('phone', phone)
    .select()
    .single();
  ...
  const result = updated || { ...user, home_currency: detected.currency, home_country: detected.country };

  await supabase.from('wallets').upsert(
    { phone, currency: detected.currency, balance: 0 },
    { onConflict: 'phone,currency', ignoreDuplicates: true }
  );

  // Drop legacy zero-balance wallets from the old multi-currency model.
  await supabase.from('wallets').delete()
    .eq('phone', phone)
    .neq('currency', detected.currency)
    .eq('balance', 0);
...
async function finalizeRegistration(phone, ctx, documentUrls) {
  ...
  await supabase
    ...
    .update({
      ...
      kyc_email:     ctx.email,
      ...(detected ? { home_currency: detected.currency, home_country: detected.country } : {}),
    })
    .eq('phone', phone);
...
async function handleTxnAmount(phone, msg, session) {
  ...
  try {
    ...
    // Cross-border — bridged through USD internally, never shown to the user.
    ...
    const totalDebit = parseFloat((amount + estimate.totalFee).toFixed(2));
    if (balance < totalDebit) {
      await setSession(phone, 'idle', {});
      return `Please add funds in your account. You need *${totalDebit.toFixed(2)} ${walletCurrency}* (incl. fees) but your balance is *${balance.toFixed(2)} ${walletCurrency}*. Reply "3" from the menu to top up.`;
    }
...
async function finalizeTransaction(phone, ctx, reference) {
  ...
  try {
    ...
    // Cross-border plain sends: lock the real FX quote immediately before
    // submitting, so the lock window between quote and use is as short as
    // possible (the estimate shown earlier was unlocked, via /business/rates).
    if (isCrossBorder) {
      const locked = await lockCrossBorderQuote({
        destCurrency: ctx.currency,
        destCountry: country,
        channelType: ctx.channelType,
        destAmount: payoutAmount,
      });
      sendQuoteId = locked.quoteId;
    }
    ...
    await supabase.from('transactions').insert({
      ...
      quote_id: sendQuoteId || null,
      // Cross-currency invoice payments bridge the wallet through
      // buildInvoiceWalletBridge (Workflow 3's bridging math) — that's the
      // rate actually relevant to "wallet currency -> payout currency"
      // shown on receipts, so it takes priority over the invoice's own
      // internal YC-settlement quote (ctx.ycRate/displayRate/marginPct,
      // a different rate concept in a different currency pair).
