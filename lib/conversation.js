...
async function finalizeTransaction(phone, ctx, reference) {
  ...
  try {
    ...
    // YC requires full sender KYC for BWP/ZAR/ZMW (Tier 0 reduced KYC does not apply).
    // See https://docs.yellowcard.engineering/docs/kyc-metadata
    const sender = {
    ...
    // Valid reasons per https://docs.yellowcard.engineering/docs/payment-reasons-api
    const reason = ctx.purpose === 'invoice_payment' ? 'bills' : 'other';
    ...
    // Safety fallback if forceAccept didn't start processing (10-min expiry otherwise)
