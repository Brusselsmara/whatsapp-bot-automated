...
function computeTxnProfit(txn) {
  ...
  if (txn.type === 'send' || txn.type === 'invoice_payment') {
    ...
    const paylinkProfit = subMoney(totalCustomerFees, ycFee);
    const fxMargin = Math.max(0, subMoney(paylinkProfit, markup));

    return { markup, fxMargin, ycFee, paylinkProfit };
  }

  return { markup: 0, fxMargin: 0, ycFee: 0, paylinkProfit: 0 };
}
  ...
  for (const txn of completedInRange) {
    ...
    if (txn.type === 'topup') {
      ...
      bucket.balance.topupPoolAfterProfit = addMoney(
        ...
        subMoney(txn.amount, profit.paylinkProfit)
      );
      bucket.balance.netCreditedToWallets = addMoney(
        bucket.balance.netCreditedToWallets,
        subMoney(txn.amount, addMoney(profit.ycFee, profit.paylinkProfit))
      );
