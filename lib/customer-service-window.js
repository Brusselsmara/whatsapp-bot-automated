...
async function assertPwaTwilioAllowed(phone) {
  ...
  if (!status.activated) {
    throw new PwaTwilioGateError(
      'PWA_NOT_ACTIVATED',
      'Message Romela Pula on WhatsApp and reply *app* to activate the web app first.'
    );
  }
  ...
  if (!status.cswOpen) {
    throw new PwaTwilioGateError(
      'CSW_CLOSED',
      'Your WhatsApp session has expired. Message Romela Pula on WhatsApp again (reply *app*), then sign in within 24 hours.'
    );
  }
...
function buildPwaActivationReply() {
  ...
  if (!url) {
    return (
      'Romela Pula app is not configured yet (PUBLIC_APP_URL missing). ' +
      'Please contact support.'
    );
  }

  return (
    `✅ *Romela Pula app activated*\n\n` +
...
async function buildPwaActivationHintForApp(phone, user) {
  ...
  if (!registered || !status.activated) {
    return (
      `Romela Pula app access is activated from *WhatsApp* only.\n\n` +
      `Open WhatsApp, message Romela Pula, and reply *app*. Then return here to sign in within 24 hours.\n\n` +
  ...
  if (!status.cswOpen) {
    return (
      `Your WhatsApp session has expired. Message Romela Pula on WhatsApp again (reply *app*), ` +
      `then sign in here within 24 hours.\n\n` +
      `Reply *menu* for your account options.`
    );
  }

  return `You're already signed in to the Romela Pula app. Reply *menu* for options.`;
}
