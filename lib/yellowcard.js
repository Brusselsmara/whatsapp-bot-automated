...
/**
 * Live conversion quote — POST /business/quotes (getConversionQuote).
 * Returns a quoteId that must be passed to submitSend before expiry (~10 min).
 */
async function getConversionQuote({ txType, localAmount, currency, country, channelType }) {
  ...
  console.log('[YC] getConversionQuote payload:', JSON.stringify(payload));
  const data = await ycRequest('POST', '/business/quotes', payload);

  const quoteId = data.quoteId || data.id;
  const rate = data.rate ?? data.rateLocal;
  const expiresAt = data.expiresAt || data.expireAt;
  const usdAmount = data.usdAmount ?? data.amount;

  if (!quoteId || rate == null) {
    throw new Error(`Invalid quote response: ${JSON.stringify(data)}`);
  }

  return { quoteId, rate: parseFloat(rate), expiresAt, usdAmount, raw: data };
}
 ...
 */
async function submitSend({ sequenceId, localAmount, country, currency, channelType, reason, sender, destination, customerUID, quoteId }) {
  const payload = {
    sequenceId,
    localAmount,
    country,
