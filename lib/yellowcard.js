...
const { proxiedFetch, assertProductionProxyIfRequired, isProxyConfigured, redactProxyUrl } = require('./outbound-proxy');
...
function logYcEnvironmentOnce() {
  ...
  const sandbox = isYcSandbox();
  console.log(`[YC] Payments API: ${sandbox ? 'SANDBOX' : 'PRODUCTION'} (${baseUrl})`);
  if (!sandbox && isProxyConfigured()) {
    console.log(`[YC] Outbound proxy: ${redactProxyUrl(process.env.FIXIE_URL || process.env.QUOTAGUARDSTATIC_URL || process.env.HTTPS_PROXY)}`);
  } else if (!sandbox) {
    console.warn('[YC] No FIXIE_URL / QUOTAGUARDSTATIC_URL — production API calls use dynamic Vercel IPs');
  }
}
...
//   Romela Pula registers users on any YC-supported corridor in COUNTRY_CONFIG.
...
function getCorridorsForPicker(opts = {}) {
  const { currency, channelType } = opts;
  let corridors = getSupportedCorridors();
  if (currency) {
    corridors = corridors.filter((c) => c.currency === String(currency).toUpperCase());
  }
  if (channelType) {
    corridors = corridors.filter((c) => (c.channelTypes || []).includes(channelType));
  }
  return corridors;
}

/** Distinct send channel types available for a currency (e.g. XOF → momo only). */
function sendChannelTypesForCurrency(currency) {
  const channels = new Set();
  getSupportedCorridors()
    .filter((c) => c.currency === String(currency).toUpperCase())
    .forEach((c) => (c.channelTypes || []).forEach((ch) => channels.add(ch)));
  return [...channels];
}

function formatCorridorPickerPrompt(title, opts = {}) {
  const corridors = getCorridorsForPicker(opts);
  if (!corridors.length) {
    const via = opts.channelType ? channelTypeLabel(opts.channelType) : 'this payment method';
    return (
      `${title}\n\n` +
      `Sorry, no destinations support ${via} for this selection right now. Reply *menu* to go back.`
    );
  }
...
function channelTypeLabel(channelType) {
  if (channelType === 'bank') return 'bank transfer';
  if (channelType === 'momo') return 'mobile money';
  return String(channelType || 'payment');
}
...
function detectCountryFromNumber(rawNumber) {
  ...
  // --- TEST OVERRIDE FOR NON-AFRICAN NUMBERS ---
  const testPhone = String(process.env.TEST_OVERRIDE_PHONE || '').replace(/\D/g, '');
  const testCountry = String(process.env.TEST_OVERRIDE_COUNTRY || 'BW').toUpperCase();
  if (testPhone && digits === testPhone && COUNTRY_CONFIG[testCountry]) {
    return { country: testCountry, currency: COUNTRY_CONFIG[testCountry].currency };
  }
  // ---------------------------------------------
...
/** True when the number's dial code matches a supported Romela Pula corridor. */
...
async function ycRequest(method, path, body, timeoutMs = 15000) {
  ...
  assertProductionProxyIfRequired(!isYcSandbox());
  ...
  let res;
  try {
    res = await proxiedFetch(`${baseUrl}${path}`, {
      method,
      headers,
      body: bodyStr,
...
function sanitizeDestinationName(name) {
  if (!name || typeof name !== 'string') return 'Romela Pula Recipient';
  const cleaned = name
    .normalize('NFKD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-zA-Z\s]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  return cleaned || 'Romela Pula Recipient';
}

/** BWP, ZAR, and NGN never qualify for Tier 0 — full KYC is always required (YC docs). */
const FULL_KYC_CURRENCIES = new Set(['BWP', 'ZAR', 'NGN']);

const YC_ID_TYPES = new Set(['license', 'passport', 'NIN']);

function requiresFullKycForCurrency(currency) {
  return FULL_KYC_CURRENCIES.has(String(currency || '').toUpperCase());
}

function isValidYcIdType(idType) {
  return YC_ID_TYPES.has(idType);
}

function sanitizeKycIdNumber(raw) {
  return String(raw || '').replace(/\s+/g, '').trim();
}

/** Parse dd/mm/yyyy or mm/dd/yyyy into a Date (local), or null if invalid. */
function parseFlexibleDob(raw) {
  ...
  // Prefer dd/mm/yyyy (registration prompt), then mm/dd/yyyy (YC format).
  return tryDate(b, a) || tryDate(a, b);
}

/** Yellow Card expects dob as mm/dd/yyyy. Returns null when invalid or in the future. */
function formatDobForYc(raw) {
  const d = parseFlexibleDob(raw);
  if (!d) return null;
...
function normalizeIdTypeForYc(raw) {
  const s = String(raw || '').toLowerCase().trim();
  if (!s) return 'license';
  if (s.includes('passport')) return 'passport';
  if (s.includes('license') || s.includes('licence') || s.includes('driver')) return 'license';
  if (s.includes('nin')) return 'NIN';
  // YC docs use "license" for generic government ID — not "national_id".
  if (s.includes('national') || s === 'id') return 'license';
  return 'license';
}

/**
 * Build retail KYC metadata for YC receive/send.
 * @param {{ strict?: boolean }} opts — strict: no fallbacks; returns null if required fields invalid
 */
function buildRetailKycFromUser(user, phone, country, opts = {}) {
  const { strict = false } = opts;
  const dob = formatDobForYc(user?.kyc_dob);
  const name = user?.kyc_name || user?.business_name;
  const address = user?.kyc_address;
  const email = user?.kyc_email;
  const idNumber = sanitizeKycIdNumber(user?.kyc_id_number);
  const idType = normalizeIdTypeForYc(user?.kyc_id_type);

  if (strict && (!name || !address || !dob || !email || !idNumber || idNumber.length < 5)) {
    return null;
  }

  return {
    name: String(name || 'Romela Pula User').trim(),
    country,
    phone: toInternationalPhone(phone, country),
    address: String(address || 'Address on file').trim(),
    dob: dob || '01/01/1990',
    email: String(email || 'noreply@romelapula.app').trim(),
    idNumber: idNumber || '000000',
    idType: isValidYcIdType(idType) ? idType : 'license',
  };
}

/** Returns field names that fail Yellow Card retail KYC validation. */
function validateRetailKycPayload(kyc) {
  const issues = [];
  if (!kyc?.name || String(kyc.name).trim().length < 2) issues.push('name');
  if (!kyc?.country) issues.push('country');
  if (!kyc?.phone || !String(kyc.phone).startsWith('+')) issues.push('phone');
  if (!kyc?.address || String(kyc.address).trim().length < 3) issues.push('address');
  if (!/^\d{2}\/\d{2}\/\d{4}$/.test(String(kyc?.dob || ''))) issues.push('date of birth');
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(kyc?.email || ''))) issues.push('email');
  const idNum = sanitizeKycIdNumber(kyc?.idNumber);
  if (!idNum || idNum.length < 5) issues.push('ID number');
  if (!isValidYcIdType(kyc?.idType)) issues.push('ID type');
  return issues;
}

function describeMissingKycFields(user) {
  const missing = [];
  if (!user?.kyc_name && !user?.business_name) missing.push('full name');
  if (!user?.kyc_address) missing.push('address');
  if (!formatDobForYc(user?.kyc_dob)) {
    missing.push('date of birth (dd/mm/yyyy, must be in the past — e.g. 15/03/1990)');
  }
  if (!user?.kyc_email) missing.push('email');
  const idNum = sanitizeKycIdNumber(user?.kyc_id_number);
  if (!idNum || idNum.length < 5) missing.push('ID number (at least 5 characters)');
  if (!user?.kyc_id_type) missing.push('ID type');
  return missing;
}
...
module.exports = {
  ...
  parseFlexibleDob,
  formatDobForYc,
  normalizeIdTypeForYc,
  requiresFullKycForCurrency,
  validateRetailKycPayload,
  buildRetailKycFromUser,
  describeMissingKycFields,
  buildAuthHeaders,
  getWebhookSignature,
