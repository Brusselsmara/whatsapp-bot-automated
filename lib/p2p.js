...
const P2P_MAX_AMOUNT = parseFloat(process.env.PAYLINK_P2P_MAX_AMOUNT || '5000');

const REASON_MESSAGES = {
  invalid_amount: 'Enter a valid amount greater than zero.',
  self_transfer: 'You cannot send money to yourself.',
  sender_not_eligible: 'Your account must be approved before sending to PayLink users.',
  recipient_not_eligible: 'That person is not on PayLink yet, or their account is not approved.',
  currency_mismatch: 'PayLink-to-PayLink transfers must be in the recipient\'s home currency for now.',
  insufficient_funds: 'Insufficient balance for this transfer.',
};

async function findPaylinkRecipient(phone) {
