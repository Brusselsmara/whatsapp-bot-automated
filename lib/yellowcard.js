...
const COUNTRY_CONFIG = {
  BW: { currency: 'BWP', channelTypes: ['bank', 'momo'], dialCode: '267' }, // Bank + MyZaka mobile money
  ZA: { currency: 'ZAR', channelTypes: ['bank'],          dialCode: '27'  }, // Bank transfer only
  ZM: { currency: 'ZMW', channelTypes: ['momo'],          dialCode: '260' }, // Mobile money only (Airtel/MTN/TNM)
...
async function submitReceive({ sequenceId, localAmount, country, channelType, currency, recipient, source }) {
  return ycRequest('POST', '/business/receive', {
    sequenceId,
    localAmount,
    country,
    currency,
...
async function getNetworks(country, accountNumberType) {
  ...
  console.log(`[YC] getNetworks: total=${Array.isArray(data) ? data.length : 'non-array'} active=${active.length} filtered(${accountNumberType})=${filtered.length}`);
  return filtered;
}
