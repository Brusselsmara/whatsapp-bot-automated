...
const { TOPUP_FLAT_FEE_BWP, PAYLINK_SEND_FEE_TIERS } = require('./fees');
...
function channelSummary(types = []) {
  ...
  if (hasBank && hasMomo) return 'Mobile money & bank';
  if (hasBank) return 'Bank only';
  if (hasMomo) return 'Mobile money only';
  return '—';
}

/**
 * Customer-facing fee schedule PDF (PayLink fees only — no third-party rail breakdown).
 * @param {object} [opts]
...
async function buildFeeSchedulePdf({ walletCurrency } = {}) {
  ...
  doc.fontSize(20).fillColor('#000').font('Helvetica-Bold').text('PayLink Fee Schedule', { align: 'center' });
  doc.moveDown(0.3);
  doc.fontSize(11).fillColor('#666').font('Helvetica').text('Top-up & send fees — mobile money and bank', { align: 'center' });
  doc.moveDown(0.2);
  ...
  heading('1. How PayLink charges you', 13);
  body(
    'PayLink shows one total fee before you confirm each top-up or send. This document explains PayLink\'s fee structure only. ' +
  ...
  body(`PayLink processing fee: BWP ${TOPUP_FLAT_FEE_BWP} flat per top-up (converted to your wallet currency at the time of top-up).`);
  bullet('Applies to mobile money and bank top-ups where available.');
  ...
  body(
    'PayLink charges a tiered service fee based on transfer size (converted to BWP equivalent for tiering), ' +
    'plus a variable processing component that depends on amount, destination country, and channel (mobile money or bank).'
  );
  doc.moveDown(0.3);
  body('PayLink tiered service fee (BWP equivalent of transfer size):');
  doc.moveDown(0.2);
  for (const tier of PAYLINK_SEND_FEE_TIERS) {
  ...
  bullet('Total debit = send amount + total PayLink fee (shown as one line before you confirm).');
  doc.moveDown(0.3);

  body('Cross-border send (e.g. BWP to ZMW):');
  bullet('You enter the total from your wallet (fees included).');
  bullet('PayLink works out recipient amount, exchange rate, and fees internally.');
  bullet('Your quote shows total you pay, approximate recipient amount, and rate.');
  doc.moveDown(0.3);

  body('Mobile money vs bank:');
  bullet('Both use the same PayLink tier table above.');
  bullet('The variable processing part can differ between mobile money and bank — your WhatsApp quote shows the exact total for your chosen channel.');

  heading('4. Invoice payments', 13);
  body(
    'Paying a supplier invoice uses the same PayLink send fee model. The supplier receives the invoice amount; ' +
  ...
  doc.fontSize(9).fillColor('#999').text(
    'PayLink — This schedule may be updated. The fee shown in WhatsApp at confirmation time applies. Reply menu for support.',
    { align: 'center' }
  );
