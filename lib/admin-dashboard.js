...
<div class="card">
  ...
  return `<!DOCTYPE html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>PayLink Admin — Sign in</title>
<style>
  body{font-family:system-ui,-apple-system,sans-serif;background:#f4f6f8;margin:0;padding:40px 16px;color:#1a1a2e}
  .card{max-width:420px;margin:40px auto;background:#fff;border-radius:12px;padding:32px;box-shadow:0 4px 24px rgba(0,0,0,.08)}
  ...
  <section class="currency-block">
    ...
    <div class="grid">
      ...
      <div class="metric">
        ...
        <ul>
          <li>Top-ups collected minus PayLink profit</li>
...
function renderTxnTable(rows) {
  ...
  const trs = rows.map((r) => {
    const payout = r.payoutAmount != null
      ...
      : '—';
    const profitParts = [
      `Total ${r.profit.paylinkProfit.toFixed(2)}`,
  ...
  return `<table>
    <thead>
      <tr>
        ...
        <th>Amount</th><th>Recipient</th><th>PayLink profit</th><th>Ref</th>
      </tr>
    </thead>
    <tbody>${trs}</tbody>
  </table>`;
...
function renderDashboardPage(data, { filterError } = {}) {
  ...
  return `<!DOCTYPE html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>PayLink Admin Dashboard</title>
<style>
