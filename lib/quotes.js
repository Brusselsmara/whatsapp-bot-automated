...
const { buildPaylinkServiceMarkup } = require('./fees');
...
function computeInvoiceCustomerFeesWallet(ctx) {
  ...
  if (!ctx.fxBridgedRate || !ctx.fxBridgeDisplayRate) {
    return parseFloat((parseFloat(ctx.ycFeeAmount || 0) + parseFloat(ctx.markupAmount || 0)).toFixed(2));
  }
  ...
  const principalWallet = face / bridgedRate;
  return parseFloat((walletDebit - principalWallet).toFixed(2));
}
...
function computeInvoiceCustomerFeesFromTxn(txn) {
  ...
  if (!isCross || !txn.display_rate) {
    return parseFloat((parseFloat(txn.yc_fee_amount || 0) + parseFloat(txn.markup_amount || 0)).toFixed(2));
  }
  ...
  return parseFloat((walletDebit - face / bridgedRate).toFixed(2));
}
...
async function buildDomesticSendFee({ country, currency, channelType, amount }) {
  ...
  const totalFee = parseFloat((ycFeeAmount + markup.markupAmount).toFixed(2));
...
async function crossBorderFeesAtPrincipal({ sourceCurrency, destCountry, destCurrency, channelType, principalFx, displayRate }) {
  ...
  const totalFee = parseFloat((ycFeeWallet + markup.markupAmount).toFixed(2));
  return {
    ycFeeAmount: ycFeeWallet,
    ycFeeDest,
