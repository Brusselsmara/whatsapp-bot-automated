...
/**
 * Check every pending topup for this user against Yellow Card and credit
 * the wallet if completed. Called fire-and-forget on every incoming message
 * so balances stay up to date without needing a cron job.
 */
async function settlePendingTopups(phone) {
  try {
    ...
    console.log(`[SETTLE] ${pending.length} pending topup(s) for ${phone}`);

    for (const txn of pending) {
      try {
        const ycData = await yc.getReceive(txn.yellowcard_reference);
        const ycStatus = (ycData?.status || '').toUpperCase();
        console.log(`[SETTLE] txn=${txn.id} ycStatus=${ycStatus}`);

        if (ycStatus === 'COMPLETE' || ycStatus === 'COMPLETED') {
          // Idempotency: re-read status in case a concurrent call already settled it
          ...
          const { data: wallet } = await supabase
            ...
            .eq('phone', phone).eq('currency', txn.currency).single();
          ...
          console.log(`[SETTLE] ✅ Credited ${txn.amount} ${txn.currency} — balance ${prev} → ${next}`);
          ...
          console.log(`[SETTLE] ❌ Topup ${txn.id} marked failed (${ycStatus})`);
        }
      } catch (err) {
        // Don't let one failed poll block the others
        console.warn(`[SETTLE] Error polling txn ${txn.id}:`, err.message);
      }
    }
  } catch (err) {
    console.warn('[SETTLE] settlePendingTopups error:', err.message);
  }
}
...
async function handleIncomingMessage(phone, text, mediaUrls = []) {
  ...
  settlePendingTopups(phone).catch(() => {});
...
async function handleTopupAccountNumber(phone, msg, session) {
  ...
  // Yellow Card requires account numbers in international format for momo channels.
  const rawAccount = msg.trim();
  const accountNumber = ctx.channelType === 'momo'
    ? yc.toInternationalPhone(rawAccount, ctx.country)
    : rawAccount;
  const source = { accountType: ctx.channelType, accountNumber };
  ...
  try {
    ...
    console.log(`[TOPUP] Using networkId: ${source.networkId}`);
    ...
    else console.log(`[TOPUP] Transaction saved — yellowcard_reference=${receive.id} status=${receive.status}`);
