...
async function buildPaylinkServiceMarkup({ tierBasisAmount, tierBasisCurrency, feeCurrency, ycFeeAmount }) {
  ...
  const ycFeeMarkup = mulMoney(ycFee, YC_FEE_PAYLINK_MARKUP_PCT);
...
/** Customer pays TOPUP_YC_FEE_MULTIPLIER × YC collection fee; PayLink keeps the excess over 1×. */
...
const TOPUP_YC_FEE_PAYLINK_MARKUP_PCT = TOPUP_YC_FEE_MULTIPLIER - 1; // 0.2 = 20% of YC fee

/**
 * PayLink top-up markup in feeCurrency:
 *   BWP 10 flat (converted) + 20% of YC collection fee.
 * Customer total fee = ycFeeAmount + markupAmount = flat + 1.2 × ycFee.
 */
async function buildPaylinkTopupMarkup({ feeCurrency, ycFeeAmount }) {
  const ycFee = parseFloat(ycFeeAmount);
  const flatFee = await flatBwpToCurrency(TOPUP_FLAT_FEE_BWP, feeCurrency);
  const ycFeeMarkup = mulMoney(ycFee, TOPUP_YC_FEE_PAYLINK_MARKUP_PCT);
  const markupAmount = addMoney(flatFee, ycFeeMarkup);
...
module.exports = {
  ...
  TOPUP_YC_FEE_PAYLINK_MARKUP_PCT,
  PAYLINK_SEND_FEE_TIERS,
