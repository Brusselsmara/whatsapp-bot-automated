...
async function settlePending(phone) {
  try {
    ...
    const phoneKeys = [...new Set([storedPhone, phone, storedPhone.replace(/^\+/, ''), phone.replace(/^\+/, '')])]
      .filter(Boolean);
  ...
  if (SEND_COMPLETE.includes(ycStatus)) {
    ...
    try {
      await notifyUser(result.phone, {
        ...
        body:
          `Your transfer of ${result.amount} ${result.currency} to ${txn.recipient_name} failed (${ycStatus.toLowerCase()}). ` +
          `Your balance has been refunded. Open the PayLink app to try again.`,
      });
    } catch (e) {
      console.error('[SETTLE] Refund notify failed:', e.message);
    }
  }
}
