...
async function settlePending(phone) {
  try {
    // YC returns status "created" on submit, then "processing", then "complete"
...
async function settleSend(txn) {
  ...
  if (['COMPLETE', 'COMPLETED', 'SUCCESS'].includes(ycStatus)) {
    ...
    // Send receipt PDF as WhatsApp media
...
Your receipt is attached.`,
    ...
    const { data: wallet } = await supabase
      ...
      .eq('phone', txn.phone).eq('currency', txn.currency).single();
    if (wallet) {
      const refunded = parseFloat(wallet.balance) + parseFloat(txn.amount);
      await supabase.from('wallets').update({
        balance: refunded, updated_at: new Date().toISOString(),
      }).eq('phone', txn.phone).eq('currency', txn.currency);
...
async function handleInvoicePayCode(phone, msg, session) {
  ...
  await setSession(phone, 'txn_amount', {
    ...
    country: CURRENCY_TO_COUNTRY[invoice.currency],
...
async function handleTxnRecipientAccount(phone, msg, session) {
  const ctx = session.context;
  // Normalise momo numbers to international format; leave bank account numbers as-is
  const recipientAccountNumber = ctx.channelType === 'momo'
    ? yc.toInternationalPhone(msg.trim(), CURRENCY_TO_COUNTRY[ctx.currency] || '')
    : msg.trim();
  await setSession(phone, 'txn_currency', { ...ctx, recipientAccountNumber });
  return `Which currency? Reply with one of: ${SUPPORTED_CURRENCIES.join(', ')}`;
}
...
async function handleTxnCurrency(phone, msg, session) {
  ...
  const { channelType } = session.context;
  ...
  await setSession(phone, 'txn_amount', { ...session.context, currency, country });
