 ...
 * country, optionally filtered to channelType 'bank' or 'momo'.
 */
async function getNetworks(country, channelType) {
  const data = await ycRequest('GET', `/business/networks?country=${country}`);
  const networks = Array.isArray(data) ? data : Array.isArray(data?.networks) ? data.networks : [];

  console.log(`[YC] getNetworks request: country=${country}, channelType=${channelType}`);
  console.log(`[YC] getNetworks response:`, JSON.stringify(data, null, 2));

  const active = networks.filter((n) => n.status === 'active');
  const filtered = channelType ? active.filter((n) => n.channelType === channelType) : active;

  console.log(`[YC] Total networks: ${networks.length}, Active: ${active.length}, Filtered: ${filtered.length}`);

  if (filtered.length === 0) {
    console.warn(`[YC] WARNING: No ${channelType || 'any'} networks found for country ${country}`);
  }
