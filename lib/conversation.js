...
async function handleInvoicePayAccount(phone, msg, session) {
  ...
  try {
    ...
    if (balance < feeQuote.totalCharge) {
      await setSession(phone, 'idle', {});
      return `Insufficient balance for this invoice.\n\nYou need *${feeQuote.totalCharge.toFixed(2)} ${ctx.currency}* (incl. fees) but only have *${balance.toFixed(2)} ${ctx.currency}*.\n\nReply *menu* → Top-up Balance to add funds.`;
    }
...
async function handleTxnAmount(phone, msg, session) {
  ...
  if (txnCtx.purpose === 'invoice_payment' && !txnCtx.invoiceCode) {
    try {
      const feeQuote = await buildInvoicePaymentQuote({
        payoutAmount: amount,
        currency: txnCtx.currency,
        country: txnCtx.country,
        channelType: txnCtx.channelType,
      });

      const { data: wallet } = await supabase
        .from('wallets').select('balance')
        .eq('phone', phone).eq('currency', txnCtx.currency).single();
      const balance = wallet ? parseFloat(wallet.balance) : 0;
      if (balance < feeQuote.totalCharge) {
        await setSession(phone, 'idle', {});
        return `Insufficient balance.\n\nYou need *${feeQuote.totalCharge.toFixed(2)} ${txnCtx.currency}* (incl. fees) but only have *${balance.toFixed(2)} ${txnCtx.currency}*. Reply *menu* → Top-up Balance.`;
      }
  ...
  const { data: wallet } = await supabase
    ...
    .select('*')
    .eq('phone', phone)
    .eq('currency', txnCtx.currency)
    .single();

  const balance = wallet ? parseFloat(wallet.balance) : 0;
  if (balance < amount) {
    await setSession(phone, 'idle', {});
    return `Please add funds in your account. Your current balance is ${balance} ${txnCtx.currency}, which isn't enough for this transaction. Reply "3" from the menu to top up.`;
  }

  try {
    const sendCtx = { ...txnCtx, amount, payoutAmount: amount };
    const { quote } = await attachLiveQuote(phone, sendCtx, user);
    return formatSendQuoteMessage(sendCtx, quote);
  } catch (err) {
    console.error('[QUOTE] Send quote error:', err);
    return `Could not fetch a live rate (${err.message}). Reply *menu* to try again.`;
  }
}
...
async function finalizeTransaction(phone, ctx, reference) {
  ...
  try {
    ...
    await supabase.from('transactions').insert({
      ...
      yc_rate: ctx.ycRate || null,
      display_rate: ctx.displayRate || null,
...
function formatTxnStatus(t) {
  ...
  const rateLine = t.display_rate
    ? `FX rate (your margin): ${parseFloat(t.display_rate).toFixed(4)}\n`
    : '';
  return (
    `📋 *Transaction Status*\n\n` +
