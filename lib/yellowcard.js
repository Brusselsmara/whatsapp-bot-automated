...
function normalizeIdTypeForYc(raw) {
  ...
  if (s.includes('national') || s === 'id') return 'national_id';
  return 'license';
}
...
function buildRetailKycFromUser(user, phone, country, opts = {}) {
  ...
  const idNumber = user?.kyc_id_number;
  const idType = normalizeIdTypeForYc(user?.kyc_id_type);

  if (strict && (!name || !address || !dob || !email || !idNumber)) {
    return null;
  }

  return {
    name: name || 'PayLink User',
    country,
    phone: toInternationalPhone(phone, country),
    address: address || 'Address on file',
    dob: dob || '01/01/1990',
    email: email || 'noreply@paylink.app',
...
function describeMissingKycFields(user) {
  ...
  if (!user?.kyc_id_number) missing.push('ID number');
