...
function buildRemittancePdf(txn) {
  return new Promise((resolve, reject) => {
    ...
    const customerFee = (parseFloat(txn.yc_fee_amount) || 0) + (parseFloat(txn.markup_amount) || 0);
    if (txn.payout_amount != null) {
      ...
      if (isCrossBorder) {
        ...
        row('Fees (included in total):', `${customerFee.toFixed(2)} ${txn.currency}`);
      } else {
        row('Fees:', `${customerFee.toFixed(2)} ${txn.currency}`);
