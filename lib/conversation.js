...
Your balance will update automatically once confirmed.`;
    ...
    // Trigger a poll after 15 s so the wallet credits quickly once the user approves the USSD prompt.
    // Fire-and-forget — don't await, we've already responded to the user.
    setTimeout(async () => {
      try {
        const pollUrl = `${process.env.PUBLIC_APP_URL}/api/poll-topups`;
        const headers = { 'Content-Type': 'application/json' };
        if (process.env.CRON_SECRET) headers['x-cron-secret'] = process.env.CRON_SECRET;
        await fetch(pollUrl, { method: 'POST', headers });
        console.log('[TOPUP] Triggered poll-topups 15 s after momo submission');
      } catch (e) {
        console.warn('[TOPUP] Could not trigger poll-topups:', e.message);
      }
    }, 15_000);
