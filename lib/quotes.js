...
const { buildRomelaPulaServiceMarkup, buildRomelaPulaTopupMarkup } = require('./fees');
const {
  roundMoney,
  addMoney,
  subMoney,
...
function formatSendQuoteMessage(ctx, quote, { includeConfirm = true } = {}) {
  ...
  lines.push(
    '',
    `⏳ *Quote valid for ${QUOTE_LOCK_MINUTES} minutes* (~${mins} min). Rate and fees refresh when you reply *1*.`,
 ...
 * YC disbursement fee + Romela Pula markups (bridged) + FX margin on the payout.
...
/**
 * Domestic send fee (same currency in and out): YC fee + tiered Romela Pula markup.
 */
...
async function buildTopupFee({ country, currency, channelType, amount }) {
  ...
  const markup = await buildRomelaPulaTopupMarkup({ feeCurrency: currency, ycFeeAmount });
