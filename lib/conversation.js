...
const COUNTRY_DISPLAY_NAMES = { BW: 'Botswana', ZA: 'South Africa', ZM: 'Zambia' };

function supportedRegistrationCountriesMessage() {
  const lines = Object.entries(yc.COUNTRY_CONFIG).map(
    ([code, cfg]) => `• *+${cfg.dialCode}* — ${COUNTRY_DISPLAY_NAMES[code]} (${cfg.currency})`
  );
  return (
    `PayLink is only available for WhatsApp numbers from:\n` +
    `${lines.join('\n')}\n\n` +
...
/** Reset KYC for users whose WhatsApp dial code is outside BW / ZA / ZM. */
...
/** Momo top-up prompt — payer number must be a momo wallet in the user's home country. */
function topupMomoNumberPrompt(country, currency) {
  const name = COUNTRY_DISPLAY_NAMES[country] || country;
  ...
  return (
    `Enter the *${name}* mobile money number you will pay from (your *${currency}* wallet).\n\n` +
    `Use a *${name}* momo number in international format (e.g. *${example}*).\n\n` +
    `A USSD prompt will be sent to that number to approve the payment.${sandboxLine}`
  );
}
...
async function handleIncomingMessage(phone, text, mediaUrls = []) {
  ...
  if (!yc.isSupportedWhatsAppNumber(phone)) {
    ...
    if (msg === '2' || lower === 'help') {
      return (
        `${supportedRegistrationCountriesMessage()}\n\n` +
        `PayLink lets you pay invoices and send money across Botswana, South Africa, and Zambia.`
      );
    }
...
async function handleTxnQuoteConfirm(phone, msg, session) {
  ...
  if (lower === '1') {
    try {
      ...
      return `Could not refresh the quote (${err.message}). Reply *1* to try again or *cancel*.`;
    }
  }
...
async function handleTopupAmount(phone, msg, session) {
  ...
  return `${feeNotice}\n\n${topupMomoNumberPrompt(ctx3.country, ctx3.currency)}`;
}
...
async function handleTopupAccountNumber(phone, msg, session) {
  ...
  // Docs: accountNumber and networkId are NOT required for bank receives in production.
  // Sandbox bank receives need source.accountNumber = 1111111111 to auto-complete.
...
Your balance will update automatically once Yellow Card confirms the deposit.${sandboxNote}`;
    }
    ...
    if (errMsg.includes('phone number') || errMsg.includes('InvalidPhoneNumber')) {
      return (
        `The number wasn't accepted as a valid *${COUNTRY_DISPLAY_NAMES[ctx.country] || ctx.country}* mobile money account.\n\n` +
        `${topupMomoNumberPrompt(ctx.country, ctx.currency)}`
      );
    }
